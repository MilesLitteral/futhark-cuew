...
module Futhark.CodeGen.Backends.CUEW.Boilerplate
  ( generateBoilerplate,
    profilingEnclosure,
...
import Futhark.CodeGen.RTS.C (cuewH, freeListH)
...
generateBoilerplate ::
  ...
  [FailureMsg] ->
  GC.CompilerM OpenCL () ()
generateBoilerplate cuew_program cuew_prelude cost_centres kernels sizes failures = do
  mapM_
    GC.earlyDecl
    [C.cunit|
      $esc:("typedef CUdeviceptr fl_mem_t;")
      $esc:(T.unpack freeListH)
      const char *cuew_program[] = {$inits:fragments, NULL};
      |]
  ...
  where
    fragments =
      [ [C.cinit|$string:s|]
        | s <- chunk 2000 $ T.unpack $ cuew_prelude <> cuew_program
      ]
  ...
  GC.publicDef_ "context_new" GC.InitDecl $ \s ->
    ( [C.cedecl|struct $id:ctx* $id:s(struct $id:cfg* cfg);|],
      [C.cedecl|struct $id:ctx* $id:s(struct $id:cfg* cfg) {
                 ...
                 ctx->error = cuda_setup(&ctx->cuda, cuew_program, cfg->nvrtc_opts);
