...
-- | Code generation for CUEW.
module Futhark.CodeGen.Backends.CUEW
  ( compileProg,
    GC.CParts (..),
    GC.asLibrary,
    GC.asExecutable,
...
import Futhark.CodeGen.Backends.CUEW.Boilerplate
...
compileProg version prog = do
  (ws, Program cuew_code cuew_prelude kernels _ sizes failures prog') <-
  ...
  commonOptions
    ++ [ Option
           { optionLongName = "dump-cuda",
             ...
             optionDescription = "Dump the embedded CUDA (CUEW) kernels to the indicated file.",
             optionAction =
         ...
         Option
           { optionLongName = "load-cuda",
             optionShortName = Nothing,
             optionArgument = RequiredArgument "FILE",
             optionDescription = "Instead of using the embedded CUDA (CUEW) kernels, load them from the indicated file.",
