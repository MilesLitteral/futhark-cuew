...
import qualified Futhark.CodeGen.Backends.CUEW as CUEW
...
-- | The @futhark cuew@ action.
compileCUEWAction :: FutharkConfig -> CompilerMode -> FilePath -> Action GPUMem
compileCUEWAction fcfg mode outpath =
  Action
    { actionName = "Compile to CUEW",
      actionDescription = "Compile to CUEW",
      actionProcedure = helper
    }
  where
    helper prog = do
      cprog <- handleWarnings fcfg $ CUEW.compileProg (T.pack versionString) prog
      ...
      case mode of
        ToLibrary -> do
          let (header, impl, manifest) = CUEW.asLibrary cprog
          liftIO $ T.writeFile hpath $ cPrependHeader header
          liftIO $ T.writeFile cpath $ cPrependHeader impl
          liftIO $ T.writeFile jsonpath manifest
        ToExecutable -> do
          liftIO $ T.writeFile cpath $ cPrependHeader $ CUEW.asExecutable cprog
          runCC cpath outpath ["-O", "-std=c99"] ("-lm" : extra_options)
        ToServer -> do
          liftIO $ T.writeFile cpath $ cPrependHeader $ CUEW.asServer cprog
          runCC cpath outpath ["-O", "-std=c99"] ("-lm" : extra_options)
